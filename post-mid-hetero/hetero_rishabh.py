...
class ExtractorHeteroMLP(nn.Module):
    def __init__(self, hidden_size, shared_config, num_edge_types):
        super().__init__()
        self.learn_edge_att = shared_config['learn_edge_att'] # whether to learn edge attention
        self.num_edge_types = num_edge_types  # number of distinct edge types
        dropout_p = shared_config['extractor_dropout_p']

        if self.learn_edge_att:
            self.feature_extractor = MLP([hidden_size*2 + num_edge_types, hidden_size * 4, hidden_size, 1], dropout=dropout_p)
        else: 
            self.feature_extractor = MLP([hidden_size * 1, hidden_size * 2, hidden_size, 1], dropout=dropout_p)

    def forward(self, emb, edge_index, batch, edge_type=None):
        """
        Parameters:
          emb: node embeddings (shape: [num_nodes, hidden_size])
          edge_index: tensor of shape [2, num_edges]
          batch: tensor mapping each node to its graph (for batching)
          edge_type: tensor of shape [num_edges], each entry is an integer representing the edge type
        """
        if self.learn_edge_att:
            col, row = edge_index
            f1, f2 = emb[col], emb[row]
            # If edge type info is provided, convert it to one-hot and concatenate.
            if edge_type is not None:
                edge_type_onehot = F.one_hot(edge_type, num_classes=self.num_edge_types).float()
                f12 = torch.cat([f1, f2, edge_type_onehot], dim=-1)
            else:
                f12 = torch.cat([f1, f2], dim=-1)
            att_log_logits = self.feature_extractor(f12, batch[col])
        else:
